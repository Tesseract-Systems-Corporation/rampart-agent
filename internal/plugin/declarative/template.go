...
// Templates use Go's text/template syntax with {{.field}} notation.
...
// MustRenderTemplate renders a template, returning the original string on error.
func MustRenderTemplate(tmpl string, data map[string]interface{}) string {
	rendered, err := RenderTemplate(tmpl, data)
	if err != nil {
		return tmpl
	}
